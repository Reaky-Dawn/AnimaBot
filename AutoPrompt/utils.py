...
DEFAULT_TAG_CATEGORY_WEIGHT = 0.5
...
_DEFAULT_TAG_WEIGHT = 0.75

_ARTIST_POST_DECAY_SMOOTHING = 15.0


def _artist_concentration(cooc: float, post_count: float) -> float:
    """共现浓度：cooc / (sqrt(post_count) + 平滑常数)。

    post 开根让浓度随作品量次线性衰减，平滑常数防止小画师因极低的
    post_count 而浓度虚高，两者共同压制百帖级小画师凭 100% 浓度
    挤掉千帖级题材大手的问题。"""
    if post_count <= 0:
        return 0.0
    return cooc / (math.sqrt(post_count) + _ARTIST_POST_DECAY_SMOOTHING)


def _artist_candidate_metrics(entry: dict, tag_weights: dict | None = None) -> tuple[float, float, float]:
    """提取画师候选的覆盖率、有效共现量与画师总帖数。

    覆盖率不再是单纯的 sources 数量：按标签类别权重（如角色 2.5、版权 2.0、
    质量 0）加权求和，角色/版权类标签对画师关联度的贡献远高于质量标签。
    sources 未在权重映射中的标签按中性权重 1.0 计；重复标签去重后只计一次。

    cooc_count 是各命中 source 共现次数的累加值，同一张作品会因命中多个
    标签被重复计入，直接使用会放大 sources 数量的影响；故除以去重后的
    source 数量得到每标签平均的有效共现量，并钳制在 post_count 以内。"""
    ...
    effective_cooc = cooc / source_count if source_count > 0 else cooc
    effective_cooc = min(effective_cooc, post_count) if post_count > 0 else effective_cooc
    return coverage, effective_cooc, post_count
...
def rank_artist_candidates(results: list[dict], top_n: int = 10, tag_weights: dict | None = None) -> list[dict]:
    ...
    更契合整体需求。次级得分不能只用浓度——那会让百帖级小画师凭高浓度
    挤掉千帖级的题材大手，而作品量大的画师风格被生成模型学习得更充分，
    故浓度按 cooc / (sqrt(post) + 平滑常数) 次线性衰减后再乘 log(cooc)
    平衡专精度与证据量。cooc 使用每标签平均的有效共现量。
    ...
    def _score(entry: dict) -> tuple:
        ...
        evidence = _artist_concentration(cooc, post_count) * np.log10(cooc + 1)
        return (coverage, evidence)

    return sorted(results, key=_score, reverse=True)[:top_n]


def sample_artist_candidate(results: list[dict], top_n: int = 10, tag_weights: dict | None = None,
                            temperature: float = 1.0) -> dict | None:
    """从高相关画师候选中按相关性加权随机抽取一个。

    候选先沿用 ``rank_artist_candidates`` 的排序逻辑限制在 top_n 内，
    再用覆盖率、共现浓度和共现量构成采样权重（覆盖率下限 +0.25 保留
    低覆盖候选的入选机会）。这样高相关候选更常被选中，但不会像固定取
    第一名一样让结果失去随机性。

    temperature 控制采样的随机性：temperature 越大权重分布越平（越接近
    均匀随机），越小越贪婪（越接近固定取第一名），1.0 为默认加权采样。
    """
    if temperature <= 0:
    ...
    for candidate in candidates:
        ...
        evidence = _artist_concentration(cooc, post_count) * np.log10(cooc + 1.0)
        weights.append((max(float(coverage), 0.0) + 0.25) * max(evidence, 0.0))

    weights = np.asarray(weights, dtype=float) ** (1.0 / temperature)
...
def _parse_tag_categories(raw_json: str | dict) -> dict[str, float]:
    ...
    if entries:
        ...
        for tag, category in payload.items():
