...
from .agent_prompts import (_ANIMA_OUTPUT_FORMAT, _ANIMA_ASSEMBLY_DIRECTIVE, _JAILBREAKER, _THINKING, _CLASSIFICATION_SYSTEM_PROMPT, _CHARACTER_SELECTION_SYSTEM_PROMPT,
                           _ARTIST_SELECTION_SYSTEM_PROMPT, _EXPAND_TAGS_SYSTEM_PROMPT, _DRAWING_REQUEST_PARSER_PROMPT, _TAG_CATEGORY_CLASSIFICATION_PROMPT, quality_tags)
...
async def _classify_tags(tags: list[str]) -> dict[str, float]:
    ...
    try:
        resp = await client_cheap.chat.completions.create(
            ...
            # 中间调用关思考提速（qwen3.8-flash 等支持；强制思考模型被拒时由 clients 重试漏斗剥参兜底）
...
async def agent(
    ...
    if en_tags:
        expanded_description = f"{expanded_description.rstrip()}{en_tags}"

    user_input_block = (
        f"【扩写后的描述】\n"
        f"含义说明：扩写后的描述是系统对用户原始描述进行语义扩展后的场景描述，用于补充画面细节与氛围。\n"
        f"{expanded_description}\n\n"
        f"【用户原始需求】\n"
        f"含义说明：用户原始需求是用户提交的原始文本，在本任务中具有最高优先级；任何扩写、检索或重组结果都不得遗漏、改写、删减或违背其中明确写出的内容。\n"
        f"{original_description}"
    )

    search_results, selected_characters = await search(zh_tags, user_input_block)
    ...
    if not has_artist_tag and tags_prompt:
        ...
        if not artist_tag:
            ...
            selected_artist_data = sample_artist_candidate(
                ...
                temperature=ARTIST_SAMPLE_TEMP
            )
    ...
    return tags_prompt, natural_prompt, expanded_description, selected_characters
